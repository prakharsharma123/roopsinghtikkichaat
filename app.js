...
function getShopStatus() {
    // NO TIMING RESTRICTION — always open for testing
...
async function saveStructuredAddress() {
    ...
    try {
        // Strategy 1: Try PIN code first (most reliable for Indian addresses)
        let lat = null, lng = null, dist = null;

        if (pin.length === 6) {
            const pinRes  = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(pin + ', India')}&limit=1&addressdetails=1`,
                { headers: { 'Accept-Language': 'en' } }
            );
        ...
        // Strategy 2: If PIN didn't work, try full area+city query
        if (!lat) {
            const res  = await fetch(
